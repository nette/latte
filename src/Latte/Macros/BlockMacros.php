	...
	public function macroBlock(MacroNode $node, PhpWriter $writer)
	{
		...
		if ($name == NULL) {
			...
			if ($node->name === 'snippet') {
				...
				$node->closingCode .= "\n</$tag>";
				$this->checkExtraArgs($node);
				return $writer->write("?>\n<$tag id=\"<?php echo \$this->global->snippetDriver->getHtmlId({$writer->formatWord($name)}) ?>\"><?php " . $enterCode);
		...
		if ($node->name === 'snippet') {
			if ($node->prefix) {
				...
				$node->attrCode = $writer->write('<?php echo \' id="\' . $this->global->snippetDriver->getHtmlId(%var) . \'"\' ?>', (string) substr($name, 1));
				return $writer->write($include, $name);
			}
			$tag = trim($node->tokenizer->fetchWord(), '<>');
			$tag = $tag ? $tag : 'div';
			$this->checkExtraArgs($node);
			return $writer->write("?>\n<$tag id=\"<?php echo \$this->global->snippetDriver->getHtmlId(%var) ?>\"><?php $include ?>\n</$tag><?php ",
