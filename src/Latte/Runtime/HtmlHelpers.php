...
final class HtmlHelpers
{
	...
	public static function triggerMigrationWarning(string $name, mixed $value): void
	{
		$source = Latte\Helpers::guessTemplatePosition();
		$message = match ($value) {
			null => "null: previously it rendered as $name=\"\", now the attribute is omitted",
			true => "true: previously it rendered as $name=\"1\", now it renders as $name=\"true\"",
			false => "false: previously it rendered as $name=\"\", now it renders as $name=\"false\"",
		};
		trigger_error("Behavior change for attribute '$name' with value $message" . ($source ? " ($source)" : '.'), E_USER_WARNING);
	}
