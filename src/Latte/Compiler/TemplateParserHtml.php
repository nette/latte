...
final class TemplateParserHtml
{
	...
	private function parseTag(): ?Node
	{
		...
		if ($this->element
			...
			$this->endName = [$endText, $endVariable, $endPosition] = $this->parseEndTag();
	...
	private function parseElement(): Node
	{
		...
		if (!$void) {
			...
			[$endText, $endVariable, $endPosition] = $this->endName ?? [null, null, null];
			$this->endName = null;
			if ($endText && ($this->element->is($endText) || $this->elementData[$this->element]->textualName === $endText)) {
				...
				$elem->position = Position::range($elem->position, $endPosition);
	...
	private function parseStartTag(&$elem = null): Html\ElementNode
	{
		...
		$elem->position = Position::range($openToken->position, $stream->consume(Token::Html_TagClose)->position);
		$lexer->popState();
		return $elem;
	}
	...
	private function parseAttribute(FragmentNode $fragment): ?Node
	{
		...
		$position = Position::range($name->position, $this->parser->getStream()->peek(-1)->position);
	...
	private function parseNAttribute(): Nodes\TextNode
	{
		...
		$elem->nAttributes[$name] = new Tag(
			...
			tokens: $tokens,
			position: Position::range($nameToken->position, $endToken->position),
	...
	private function parseComment(): Html\CommentNode
	{
		...
		$node->position = Position::range($openToken->position, $closeToken->position);
		$lexer->popState();
		return $node;
	}
