...
test('text attributes', function () use ($latte) {
	...
	Assert::same(
		...
		$latte->renderToString('<button title={=false}>Button</button>'),
	);
	Assert::same(
		'<button >Button</button>',
		$latte->renderToString('<button title={=null}>Button</button>'),
	);

	Assert::same(
		'<button title="">Button</button>',
		$latte->renderToString('<button title={=""}>Button</button>'),
	);
});
...
echo $latte->compile($template);
echo $latte->renderToString($template);


Assert::same('<input disabled>', $latte->renderToString('<input disabled={=true}>'));
*/
