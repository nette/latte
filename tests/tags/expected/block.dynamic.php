...
';
		...
		echo "\n";
		$this->renderBlock((LR\Helpers::stringOrNull($ʟ_tmp = $name . '') ?? throw new InvalidArgumentException(sprintf('Block name must be a string, %s given.', get_debug_type($ʟ_tmp)))), ['var' => 40] + [], 'html') /* line %d%:%d% */;
		echo "\n";
		$this->addBlock($ʟ_nm = (LR\Helpers::stringOrNull($ʟ_tmp = "word{$name}") ?? throw new InvalidArgumentException(sprintf('Block name must be a string, %s given.', get_debug_type($ʟ_tmp)))), 'html', [[$this, 'blockWord_name']], 0);
		$this->renderBlock($ʟ_nm, get_defined_vars());
		echo '

';
		$this->addBlock($ʟ_nm = (LR\Helpers::stringOrNull($ʟ_tmp = "strip{$name}") ?? throw new InvalidArgumentException(sprintf('Block name must be a string, %s given.', get_debug_type($ʟ_tmp)))), 'html', [[$this, 'blockStrip_name']], 0);
...
';
		$this->addBlock($ʟ_nm = (LR\Helpers::stringOrNull($ʟ_tmp = rand() < 5 ? 'a' : 'b') ?? throw new InvalidArgumentException(sprintf('Block name must be a string, %s given.', get_debug_type($ʟ_tmp)))), 'html', [[$this, 'blockRand_5_a_b']], 0);
		$this->renderBlock($ʟ_nm, get_defined_vars());
	}
