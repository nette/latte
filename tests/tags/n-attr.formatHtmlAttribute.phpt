...
test('data attributes', function () {
	...
	Assert::error(
		...
		E_USER_WARNING,
		'Behavior change for attribute \'data-foo\' with value array: previously it rendered as data-foo="val1 val2", now the attribute is JSON-encoded.',
	);
});
...
test('edge cases', function () {
	Assert::same('foo="NAN"', NAttrNode::formatHtmlAttribute('foo', NAN));
