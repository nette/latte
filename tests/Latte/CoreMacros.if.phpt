...
}, 'Latte\CompileException', 'Modifiers are not allowed in {if}');

Assert::exception(function () use ($compiler) {
	$compiler->expandMacro('ifset', '$var', '|filter');
}, 'Latte\CompileException', 'Modifiers are not allowed in {ifset}');

Assert::exception(function () use ($compiler) {
	$compiler->expandMacro('else', 'if args');
}, 'Latte\CompileException', 'Arguments are not allowed in {else}, did you mean {elseif}?');

Assert::exception(function () use ($compiler) {
	$compiler->expandMacro('else', 'args');
}, 'Latte\CompileException', 'Arguments are not allowed in {else}');
