...
}, Latte\CompileException::class, 'Unknown macro {color:blue} (in JavaScript or CSS, try to put a space after bracket or use n:syntax=off)');

Assert::exception(function () use ($latte) {
	$latte->compile('<script>if (true) {return}</script>');
}, Latte\CompileException::class, 'Unknown macro {return} (in JavaScript or CSS, try to put a space after bracket or use n:syntax=off)');
