...
test(function () { // inline modifiers
	Assert::same('$template->mod(@)',  formatArgs('(@|mod)'));
	Assert::same('$template->mod3($template->mod2($template->mod1(@)))',  formatArgs('(@|mod1|mod2|mod3)'));
	Assert::same('$template->mod3($template->mod2($template->mod1(@)))',  formatArgs('((@|mod1)|mod2|mod3)'));
	Assert::same('$template->mod(@, 1, 2, $var["pocet"])',  formatArgs('(@|mod:1:2:$var["pocet"])'));
	Assert::same('$template->mod(@, 1, 2, $var["pocet"])',  formatArgs('(@|mod,1,2,$var["pocet"])'));
	Assert::same('$template->mod(@, $var, 0, -0.0, "s\"\'tr", \'s"\\\'tr\')',  formatArgs('(@|mod, $var, 0, -0.0, "s\"\'tr", \'s"\\\'tr\')'));
	Assert::same('$template->mod(@, array(1))',  formatArgs('(@|mod: array(1))'));
	Assert::same('$template->mod($a ? $b : NULL)',  formatArgs('($a ? $b|mod)'));
